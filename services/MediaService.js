...
class MediaService {
  ...
  async pickImage(options = {}) {
    try {
      ...
      if (source === 'camera') {
        result = await ImagePicker.launchCamera({ ...pickerOptions, source });
      } else {
        result = await ImagePicker.launchImageLibrary({ ...pickerOptions, source });
      }
  ...
  async pickVideo(options = {}) {
    try {
      ...
      if (source === 'camera') {
        videoResult = await ImagePicker.launchCamera({ ...pickerOptions, source });
      } else {
        videoResult = await ImagePicker.launchImageLibrary(pickerOptions);
      }
      ...
      if (videoResult.errorCode) {
        throw new Error(`Video selection error: ${videoResult.errorMessage}`);
      }
      
      // Get the selected assets
      const assets = videoResult.assets || [];
  ...
  async compressVideo(video, options = {}) {
    try {
      ...
      const bitrateParam = opts.bitrate || (() => {
        switch (opts.quality) {
          case 'high':
            return '4000k';
          case 'low':
      ...
      const command = `-i "${video.uri}" -c:v libx264 -b:v ${bitrateParam} -c:a aac -strict -2 "${outputPath}"`;
   ...
   * @param {string} [options.endpoint] The API endpoint for uploading
   * @param {boolean} [options.compress] Whether to compress the media before uploading
   * @param {object} [options.metadata] Additional metadata to include with the upload
   * @param {function} [options.progressCallback] Callback function for upload progress
   * @returns {Promise<object>} The uploaded media data
   */
