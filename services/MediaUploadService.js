...
if (!AnalyticsService) {
  throw new Error('AnalyticsService is not defined');
}
...
class MediaUploadService {
  ...
  async capturePhoto(options = {}) {
    ...
    try {
      ...
      if (result.errorCode) {
        throw new Error(`Image capture error (${result.errorCode}): ${result.errorMessage}`);
      }
  ...
  async recordVideo(options = {}) {
    ...
    try {
      ...
      if (result.errorCode) {
        throw new Error(`Video recording error (code ${result.errorCode}): ${result.errorMessage}`);
      }
  ...
  async pickImages(options = {}) {
    ...
    try {
      ...
      if (result.errorCode) {
        throw new Error(`Image selection error (${result.errorCode}): ${result.errorMessage}`);
      }
  ...
  async pickVideos(options = {}) {
    ...
    try {
      ...
      if (result.errorCode) {
        throw new Error(`Video selection error (code ${result.errorCode}): ${result.errorMessage}`);
      }
  ...
  async resizeImage(uri, maxWidth = 1080, maxHeight = 1080, quality = 80) {
    try {
      ...
      console.error(`Error resizing image (code ${error.code}):`, error.message);
      throw new Error(`Error resizing image (code ${error.code}): ${error.message}`);
    }
  }
  ...
  async cropImage(options = {}) {
    try {
      ...
      console.error(`Error cropping image (code ${error.code}):`, error);
      throw new Error(`Error cropping image (code ${error.code}): ${error.message}`);
    }
  }
  ...
  async generateVideoThumbnail(videoUri, options = {}) {
    try {
      ...
      console.error(`Error generating video thumbnail (code ${error.code}):`, error.message);
      throw new Error(`Error generating video thumbnail (code ${error.code}): ${error.message}`);
    }
  }
  ...
  async processVideo(videoUri, options = {}) {
    try {
      ...
      console.error(`Error processing video (code ${error.code}):`, error.message);
      throw new Error(`Error processing video (code ${error.code}): ${error.message}`);
    }
  }
  ...
  async uploadFile(file, path, onProgress = null) {
    ...
    try {
      ...
      const fileName = file.name || `file_${uuidv4()}`;
      ...
      console.error(`Error uploading file (code ${error.code}):`, error.message);
      ...
      throw new Error(`Error uploading file (code ${error.code}): ${error.message}`);
    }
  }
