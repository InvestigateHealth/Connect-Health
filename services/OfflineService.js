...
class OfflineQueueService {
  /**
   * Creates an instance of OfflineQueueService.
   */
  constructor() {
  ...
  async init() {
    ...
    try {
      ...
      try {
        ...
        this.isOnline = false; // Default to offline if fetch fails
      }
   ...
   * @param {string} queueKey - Queue key to load
      if (queueJson) {
        try {
          ...
          console.error(`Error parsing queue JSON for ${queueKey}:`, error);
          this.queues[queueKey] = [];
        }
      } else {
        this.queues[queueKey] = [];
  ...
  async saveQueue(queueKey) {
    try {
      ...
      let queueString;
      try {
        queueString = JSON.stringify(queue);
      } catch (stringifyError) {
        console.error(`Error stringifying queue ${queueKey}:`, stringifyError);
        return;
      }
      await AsyncStorage.setItem(queueKey, queueString);
    } catch (error) {
  ...
  async syncQueues() {
    ...
    try {
      ...
      /**
       * Check if there are any pending operations in any queue
       * 
    ...
    /**
     * Update references in other queues when a temp ID is replaced with a real ID
     * 
     * @param {string} oldId - Temporary ID
     * @param {string} newId - Real ID from the server
     */
    async updateReferencesInQueues(oldId, newId) {
      // Update comments that reference this post
      this.queues[QUEUE_KEYS.COMMENT] = this.queues[QUEUE_KEYS.COMMENT].map(operation => {
        if (operation.data.postId === oldId) {
          return {
            ...operation,
            data: {
              ...operation.data,
      ...
      // Update likes that reference this post
      this.queues[QUEUE_KEYS.LIKE] = this.queues[QUEUE_KEYS.LIKE].map(operation => {
        if (operation.data.postId === oldId) {
          return {
            ...operation,
            id: `${newId}_${operation.data.userId}`,
            data: {
              ...operation.data,
              postId: newId
            }
    ...
    /**
     * Sync a specific queue with retry logic
     * 
     * @param {string} queueKey - Queue to sync
     * @param {Function} processFn - Function to process each queue item
     */
    async syncQueue(queueKey, processFn) {
      const queue = [...this.queues[queueKey]];
      const successfulOps = [];
      ...
      for (const operation of queue) {
        try {
          ...
          const result = await withRetry(() => processFn(operation), {
            retries: 3,
            delayMs: 1000
          });
          ...
          console.error(`Error syncing queue item ${operation.id}:`, error);
          
          // If this is a dependency issue, we'll retry in the next sync
          if (error.message && error.message.includes('not yet synced')) {
            continue;
          }
          
          // For other errors, we'll mark it for removal to avoid an infinite loop
          // and log an analytics event
          Analytics.logEvent('offline_sync_operation_failed', {
            queue_type: queueKey,
      ...
      if (successfulOps.length > 0) {
        this.queues[queueKey] = this.queues[queueKey].filter(op => !successfulOps.includes(op.id));
        await this.saveQueue(queueKey);
      }
    }
  }
  ...
  // Hook for components to use the offline functionality
  export function useOfflineStatus() {
    const dispatch = useDispatch();
    const isOffline = useSelector(state => state.network.isOffline);
    const [pendingOperations, setPendingOperations] = useState(false);
    ...
    useEffect(() => {
      ...
      const startListener = OfflineQueue.registerSyncCallback('start', () => {
        setIsSyncing(true);
      });
      
      const completeListener = OfflineQueue.registerSyncCallback('complete', () => {
        setIsSyncing(false);
        setPendingOperations(OfflineQueue.hasPendingOperations());
      });
      
      const errorListener = OfflineQueue.registerSyncCallback('error', () => {
        setIsSyncing(false);
        setPendingOperations(OfflineQueue.hasPendingOperations());
      });
