...
export const fetchUpcomingEvents = createAsyncThunk(
  ...
  async ({ condition, limit = DEFAULT_LIMIT, blockedUsers = [] }, { getState, rejectWithValue }) => {
    try {
...
export const fetchUserHostedEvents = createAsyncThunk(
  ...
  async ({ userId, limit = DEFAULT_LIMIT, includePast = false }, { getState, rejectWithValue }) => {
    try {
      const { lastDoc } = getState().events;
    ...
    builder
      ...
      .addCase(fetchUpcomingEvents.fulfilled, (state, action) => {
        ...
        if (action.meta.arg.reset) {
          ...
          state.upcomingEvents = state.upcomingEvents.concat(action.payload.events);
        }
      ...
      .addCase(fetchPastEvents.fulfilled, (state, action) => {
        ...
        if (action.meta.arg.reset) {
          ...
          state.pastEvents = state.pastEvents.concat(action.payload.events);
        }
      ...
      .addCase(fetchUserHostedEvents.fulfilled, (state, action) => {
        ...
        if (action.meta.arg.reset) {
          ...
          state.userHostedEvents = state.userHostedEvents.concat(action.payload.events);
        }
      ...
      .addCase(fetchUserAttendingEvents.fulfilled, (state, action) => {
        ...
        if (action.meta.arg.reset) {
          ...
          state.userAttendingEvents = state.userAttendingEvents.concat(action.payload.events);
        }
        ...
        if (attending) {
          ...
          if (!isInAttendingList) {
            ...
            if (event) {
              ...
              state.userAttendingEventsUnsorted = true;
            }
          } else {
          ...
          state.userAttendingEvents = state.userAttendingEvents
            .filter(event => event.id !== eventId || event.hostId === userId || attending);
        }
