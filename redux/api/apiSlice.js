  ...
  baseQuery: fetchBaseQuery({
    ...
    prepareHeaders: (headers) => {
      try {
        ...
        console.error('Failed to prepare headers in apiSlice.js:', error.message, error);
    ...
    toggleLike: builder.mutation({
      queryFn: async ({ postId, userId }) => {
        try {
          ...
          return { data: { postId, userId, isLiked: !isLiked } };
        } catch (error) {
          return { error: error.message };
        }
      },
    ...
    blockUser: builder.mutation({
      queryFn: async ({ userId, blockedUserId, reason }) => {
        try {
          ...
          await firestore().collection('blockedUsers').add(blockRecord);
          
          return { data: blockRecord };
        } catch (error) {
          return { error: error.message };
        }
      },
