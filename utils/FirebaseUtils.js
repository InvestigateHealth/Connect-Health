...
class FirebaseUtils {
  ...
  async hasPendingOperations() {
    try {
      if (!this.cachedKeys) {
        this.cachedKeys = await AsyncStorage.getAllKeys();
      }
      
      // Look for keys that indicate pending operations
      return this.cachedKeys.some(key => 
  ...
  async loadOfflineQueue() {
    try {
      ...
      if (queueData) {
        ...
        // Validate that savedQueue is an array
        if (Array.isArray(savedQueue)) {
          ...
          console.error('Invalid offline queue data format');
        }
      }
    } catch (error) {
    ...
    try {
      if (this.offlineQueue.length > 0) {
        ...
        try {
          ...
          console.error('Error removing offline queue:', error);
        }
      }
    } catch (error) {
  ...
  async syncPendingShares() {
    try {
      ...
      // Validate that pendingShares is an array
      if (!Array.isArray(pendingShares)) {
        console.error('Invalid pendingShares data format');
        return;
      }
  ...
  async processPendingUpdates(collection) {
    try {
      ...
      // Validate that pendingUpdates is an object
      if (typeof pendingUpdates !== 'object' || pendingUpdates === null) {
        console.error('Invalid pendingUpdates data format');
        return;
      }
  ...
  async processPendingDeletions(collection) {
    try {
      ...
      if (!Array.isArray(pendingDeletions)) {
        console.error(`Invalid pending deletions format for collection: ${collection}`);
        pendingDeletions = [];
      }
      const processedIds = [];
  ...
  async processOperation(operation) {
    try {
      ...
      async processPendingUploads() {
        // Implementation for pending uploads
        console.log("Processing pending uploads");
        // Add implementation here
      }
    }
    
    // Export the FirebaseUtils instance
