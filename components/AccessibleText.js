...
const AccessibleText = ({
  ...
  const getAccessibilityStyle = () => {
    ...
    // Apply font scaling
    accessibilityStyles.fontSize = baseStyle.fontSize * fontScale;
    
    // Apply bold text if needed and not already a heading
    if (boldText && !headingVariants.includes(variant)) {
    ...
    if (highContrast) {
      ...
      accessibilityStyles.color = theme.colors.highContrast;
  ...
  const getAccessibilityProps = () => {
    return {
      accessible: true,
      accessibilityRole: headingVariants.includes(variant) ? 'header' : 'text',
      allowFontScaling,
      ...props
    };
  };
  const generateTestID = (variant) => `accessible-text-${variant}`;

  return (
    <Text
      testID={testID || generateTestID(variant)}
...
/**
 * Default export of AccessibleText component.
 * This component is used to render text that respects user accessibility settings.
 */
